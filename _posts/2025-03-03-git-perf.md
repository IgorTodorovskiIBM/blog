...
As an open source developer for the [zopen community](https://zopen.community/), I rely heavily on Git for version control.  But, as repositories grow in size and history, Git operations become slow to the point where they can impact our daily workflow, slow down CI/CD pipelines, and slow down our entire development process.

I wanted to make sure I was doing everything I can do optimize my Git worfklow, So recently, I investigated various Git performance optimization techniques, and I decided to put them to the test.
...
Repositories can sometimes accumulate large binary files over time. If these binaries are not needed for every operation or during development, you can instruct Git to filter these out during the cloning process. This helps in reducing bandwidth, disk space and clone time. The `--filter=blob:none` option prevents Git from downloading any large file blobs.
...
#### Sparse Checkouts: Clone What You Need
