...
As an active contributor to the [z/OS Open Tools](https://github.com/ZOSOpenTools) Github organization, I specialize in porting open-source tools to z/OS. Many of the tools the z/OS Open Tools organization ports are written in C or C++. Porting C/C++ software to z/OS can be challenging due to the various differences between Linux systems and z/OS, including C runtime differences (Glibc vs z/OS C Runtime Library), file system differences (z/OS UNIX/datasets on z/OS), security differences (RACF/SAF on z/OS), and codepages differences (EBCDIC/ASCII program support in z/OS).

As an active contributor to the [z/OS Open Tools](https://github.com/ZOSOpenTools) GitHub organization, my focus lies in porting open-source tools to z/OS. These tools, predominantly written in C or C++, present distinct challenges due to differences between Linux systems and z/OS. These differences include variations in C runtime libraries (Glibc vs. z/OS C Runtime Library), differences in the file system (z/OS UNIX/datasets on z/OS), security differences (RACF/SAF on z/OS), and variations in codepages (EBCDIC/ASCII program support in z/OS).

As the number of applications and libraries ported to z/OS increased, we continued to face a recurring issue — essential C runtime APIs were missing and creating custom workarounds for each application didn't seem like the best solution.

Additionally, z/OS UNIX supports file tags that describe a file's encoding content using metadata. This feature doesn't exist in Linux, but it's crucial in z/OS because programs can run in either EBCDIC or ASCII mode. What makes file tags interesting is that, combined with the z/OS Enhanced ASCII services, z/OS can perform automatic conversion of files to and from the program's encoding upon read and write. This enables EBCDIC and ASCII programs to interpret file data correctly, irregardless of the codepage. This simplifies handling various codepages in applications. However, the issue is that files can also have no tags. How does an application handle such "untagged" files?

That's where ZOSLIB jumps in — it addresses many of these complexities.

## What is ZOSLIB?

At its core, ZOSLIB is an open source extension of the z/OS LE C Runtime Library, offering additional functionalities:
...
* Enhanced File Operations: Improved implementations for C open, pipe, and more facilitate seamless integration with z/OS auto-conversion facilities.
...
ZOSLIB was originally introduced as a library to aid in porting Node.js to z/OS. It has since been open-sourced and is now used by over 100 open-source projects under the z/OS Open Tools organization.

The ZOSLIB project is available on GitHub at https://github.com/ibmruntimes/zoslib. It is licensed under the Apache 2.0 license, which imposes few restrictions and facilitates modification and redistribution.
...
So why is this an issue? This creates a problem because the application can not determine the true encoding of untagged files. This means that an EBCDIC-based program will interpret the file data one way and an ASCII-based program will interpret the data another way.
...
This release provides the libraries (static and dynamic), include headers files, man pages, and environment variables for use in the zopen framework.
...
First, download a stable release of zoslib from https://github.com/ZOSOpenTools/zoslibport/releases. If you have z/OS Open Tools package manager, `zopen`, on your system (details here), you can simply run:
```
...
int main() {
    ...
    // Open the file in write mode, creating it if it doesn't exist and truncating it to 0 length
...
Let's compile this using clang as follows:
```
clang -fzos-le-char-mode=ascii -m64 a.c
./a.out
ls -lT output.txt
```
...
Now let's examine a simpler way of integrating ZOSLIB, and that is by leveraging the z/OS Open Tools framework.
...
Simply adding zoslib as a dependency will instruct the build to pass in the zoslib flags. These flags automatically instruct the compiler to pick up the necessary header files and to link to the ZOSLIB static archive. If you're wondering, these flags are passed in via the .appenv file from your ZOSLIB_HOME installation. The file is generated by the zoslibport build configuration: https://github.com/ZOSOpenTools/zoslibport/blob/4bf40195e8c646d34b3fdc6299c0f00a3f9442f1/buildenv#L45

The `zopen` build framework sets the CPPFLAGS, CXXFLAGS, CFLAGS, LIBS, LDFLAGS accordingly. Fortunately, almost every project we deal with relies on these flags, meaning that we can easily pass integrate ZOSLIB as an additional library to any port.
...
# Special Thanks
Thank you to Mike Fulton, Gaby Baghdadi, Wayne Zhang, Sean Perry, Eric Janssen, and many others for their contributions to ZOSLIB.
